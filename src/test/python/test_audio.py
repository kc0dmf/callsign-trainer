...
def test_medium_letters_high():
    for x in range(1,5):
        callsign.playFirstLetterHigh2(x, AUDIO_LOCN_BASE + callsign.AUDIO_SPEED_MID)

def test_medium_letters_mid():
    for x in range(1,6):
        callsign.playLetterMid2(x, AUDIO_LOCN_BASE + callsign.AUDIO_SPEED_MID)

def test_medium_letters_low():
    for x in range(1,6):
        callsign.playLetterLow2(x, AUDIO_LOCN_BASE + callsign.AUDIO_SPEED_MID)

def test_medium_numbers():
    for x in range(0,10):
        callsign.playNumberMid2(x, AUDIO_LOCN_BASE + callsign.AUDIO_SPEED_MID)
